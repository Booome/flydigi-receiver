...
static int set_announce_data(void) {
  ...
  if (memcpy_s(&announce_data[idx], ADV_DATA_LEN_MAX - idx, &disc_level,
               sizeof(disc_level)) != EOK) {
    return -1;
  }
  ...
  if (memcpy_s(&announce_data[idx], ADV_DATA_LEN_MAX - idx, &access_mode,
               sizeof(access_mode)) != EOK) {
    return -1;
  }
  ...
  if (memcpy_s(&seek_rsp_data[rsp_idx], ADV_DATA_LEN_MAX - rsp_idx, name,
               name_len) != EOK) {
    return -1;
  }
  rsp_idx += name_len;
...
static void conn_state_changed_cb(uint16_t conn_id, const sle_addr_t *addr,
  ...
  if (conn_state == SLE_ACB_STATE_DISCONNECTED &&
    ...
    osal_task *t = osal_kthread_create((osal_kthread_handler)re_announce_task,
                                       0, "reann", 0x1000);
    if (t != NULL) {
      osal_kfree(t);
    }
