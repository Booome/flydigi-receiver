...
static void seek_result_cb(sle_seek_result_info_t *result) {
  ...
  if (!g_target_locked &&
    ...
    memcpy_s(&g_target_addr, sizeof(g_target_addr), &result->addr,
...
static void pair_complete_cb(uint16_t conn_id, const sle_addr_t *addr,
                             errcode_t status) {
...
static void auth_complete_cb(uint16_t conn_id, const sle_addr_t *addr,
...
static void
conn_param_update_cb(uint16_t conn_id, errcode_t status,
                     const sle_connection_param_update_evt_t *param) {
  osal_printk("[conn] param update result: id:%u status:0x%x interval:%u "
              "latency:%u superv:%u\r\n",
              conn_id, status, (param ? param->interval : 0),
              (param ? param->latency : 0), (param ? param->supervision : 0));
}
...
static void sle_enable_cb(uint8_t status) {
  ...
  if (sle_get_local_addr(&la) == ERRCODE_SUCC) {
    osal_printk("[conn] local addr: %02x:%02x:%02x:%02x:%02x:%02x type:%d\r\n",
                la.addr[0], la.addr[1], la.addr[2], la.addr[3], la.addr[4],
                la.addr[5], la.type);
  }
...
void axk_main(void) {
  ...
  osal_kthread_lock();
  task_handle = osal_kthread_create((osal_kthread_handler)scan_task, 0,
                                    "scan_task", 0x1000);
  if (task_handle != NULL) {
