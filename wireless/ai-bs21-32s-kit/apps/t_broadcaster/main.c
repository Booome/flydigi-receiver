...
static int set_announce_data(void) {
    ...
    if (memcpy_s(&announce_data[idx], ADV_DATA_LEN_MAX - idx, &disc_level, sizeof(disc_level)) !=
        EOK) {
        return -1;
    }
    ...
    if (memcpy_s(&announce_data[idx], ADV_DATA_LEN_MAX - idx, &access_mode, sizeof(access_mode)) !=
        EOK) {
        return -1;
    }
