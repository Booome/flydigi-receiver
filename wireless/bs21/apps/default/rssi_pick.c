...
static uint32_t rssi_now_ms(void) { return (uint32_t)uapi_systick_get_ms(); }
...
static void cand_push(cand_t *c, const uint8_t addr[SLE_ADDR_LEN],
                      int8_t rssi) {
  if (c->n == 0) {
...
bool rssi_pick_feed(const uint8_t addr[SLE_ADDR_LEN], int8_t rssi) {
  ...
  if (g_take.n != 0 &&
      rssi_now_ms() - g_take.last_seen_ms > RSSI_SWITCH_HOLD_MS) {
    memset(&g_take, 0, sizeof(g_take));
  }
