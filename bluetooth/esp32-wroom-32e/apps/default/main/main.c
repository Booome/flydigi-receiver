                                ...
                                int32_t event_id, void *event_data)
{
    ...
    case ESP_HIDH_OPEN_EVENT:
        ...
        if (param->open.status == ESP_OK) {
            ...
            printf(" transport=%s status=0x%x\n",
                   transport_str(transport), (unsigned)param->open.status);
            reset_to_scan();
        }
        break;
    case ESP_HIDH_INPUT_EVENT:
        ...
        print_bda(bda);
        printf(" transport=%s len=%d data=",
               transport_str(transport), param->input.length);
...
void app_main(void)
{
    ...
    while (1) {
        ...
        } else if (bda_eq(best->bda, g_candidate.bda) &&
                   best->transport == g_candidate.transport) {
