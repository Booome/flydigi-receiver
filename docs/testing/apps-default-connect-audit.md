...
> **任意情况下（不论以上 24 个场景覆盖与否），用户长按手柄配对键做一次 re-pair，模块必须在合理时间内重新连上。**

含义：
- 用户的**唯一不可省**的恢复操作 = "手柄长按配对键"（re-pair 信号）。
- 模块必须**始终能正确响应 re-pair 信号**：手柄进入 pairing_discoverable 时，模块**必须**在 inquiry / probe 窗口命中并走全新 SSP 连上。
- 推论：
  - 模块**任何**状态（除 CONNECTED）都必须保持 inquiry 探测（不能因内部状态错误停止 inquiry）。
  - **#3 re-pair 路径必须永远工作**——这是兜底契约，不允许被任何后续 fix 退化。
- 死锁收敛点（§四）即便无法自动恢复，**只要用户 re-pair 一次就能解**（模块 NVS 需不需要清掉旧的"双方各持无效 key"由设计决定，但 re-pair 必须通）。
