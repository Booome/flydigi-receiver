...
**默认行为 = 不写注释。** 生成/修改代码时不要"顺手补注释"，包括：单位换算说明（×1.28s 这类）、
函数契约复述、参数来源、时序描述、"why"小纸条——这些全是废话。

只有同时满足以下两条才允许写注释：
1. 删掉后**第一次读这段代码的人会做错**（不是"读得慢"，是会改错/删错）；
2. ≤2 行，且内容无法从标识符/周围代码推出。

典型合格：SPDX 头（法律）；`ESP_ERROR_CHECK` 会 abort 的 benign-INVALID_STATE 分支为何不能
用 CHECK；一行结论 + spec 章节指针。
典型不合格（本仓库反复犯过）：魔数单位换算、"caller must hold mutex"、状态机解说、
延迟归因、事故复盘、协议机制推演——后四类**必须**写进 `docs/` spec，代码里最多一行指针。

**自检闸门**：每次 commit 前，把 diff 里**新增的每一行注释**逐条质问"不写会出事故吗"，
不会就删。历史上"保留非显而易见的 why"式授权被滥用过两次（14 行论文注释、参数改动配 3 行
理由），故本节不再给允许清单，只留默认不写。
